...
## Amazon Bedrock Agent Architecture with Strands

### Strands Framework

**Strands** is a Python framework that simplifies building Amazon Bedrock Agents. Instead of manually configuring agents via AWS APIs, Strands provides a high-level, Pythonic interface for creating agents with prompts, tools, and knowledge bases.

**Benefits of Using Strands**:
- Simplified agent creation with Python code
- Easy tool (action group) definition using decorators
- Built-in support for Knowledge Bases
- Local testing capabilities before deployment
- Automatic OpenAPI schema generation for tools
- Streamlined deployment to AWS
